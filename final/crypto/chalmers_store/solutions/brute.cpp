...
unsigned long long powerhash(const std::string& s, int k)
{
    return hashonce(s, magic[k]);
}

void brute_collision()
{
    while (true)
    {
        string s = string(8, 'A');
        for (int i = 0; i < 8; i++) s[i] = rand() % 26 + 'a';
        string p = string(8, 'A');
        for (int i = 0; i < 8; i++) p[i] = rand() % 26 + 'a';

        if (s != p && powerhash(s, 0) == powerhash(p, 0))
        {
            return;
        }
    }
...
int32_t main()
{
    ...
    /*dread(string, s);
    dread(string, s2);
    cout << powerhash(s, 0) << " " << powerhash(s, 1) << " " << powerhash(s, 2) << " " << powerhash(s, 3) << "\n";
    cout << powerhash(s2, 0) << " " << powerhash(s2, 1) << " " << powerhash(s2, 2) << " " << powerhash(s2, 3) << "\n";*/

    unordered_map<ull, string> seen;

    rep(i, 1e7)
    {
        string s = string(11, 'A');
        rep(i, 11) s[i] = randint('a', 'z');
        seen[powerhash(s)] = s;
    }
